...
const { data: todos = [], isLoading, error } = useQuery({
  queryKey: ["todos", token],           // token もキーに入れると取り違え防止
  queryFn: async () => {
    const res = await http.get("/api/todos", {
      headers: { "Cache-Control": "no-cache" }, // ← 304回避（CloudFrontが残っても無視）
    });
    const d = res.data;
    if (Array.isArray(d)) return d;           // 期待通り（配列）
    if (d && Array.isArray((d as any).items)) return (d as any).items; // {items:[...]}系
    return [];                                 // どれでもなければ空配列に正規化
  },
  enabled: !!token,            // 未ログイン時は叩かない
  staleTime: 0,                // 取得後すぐ古い扱い（毎回取りに行く）
  gcTime: 0,                // メモリキャッシュを持たない（デバッグ中は特に）
});
